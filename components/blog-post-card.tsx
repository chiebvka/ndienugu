...
export default function BlogPostCard({ post }: BlogPostCardProps) {
  return (
    <div className="bg-card rounded-lg shadow-md overflow-hidden h-full flex flex-col group hover:shadow-xl transition-shadow duration-300">
      {post.cover_image && (
        <Link href={`/blog/${post.slug}`} aria-label={`Read more about ${post.title}`} className="block relative h-48 sm:h-56 overflow-hidden">
            <Image
